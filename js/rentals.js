...
export function createRentalsController({
    ...
    function renderKpiCards(summary, yearTax) {
        ...
        target.innerHTML = `
            ...
            <div class="rental-kpi">
                <span>Mój przychód</span>
                <strong>${money(summary.ownerIncomeTotal)}</strong>
            </div>
            <div class="rental-kpi">
                <span>Podatek roczny</span>
                <strong>${money(yearTax?.tax || 0)}</strong>
            </div>
        `;
    }
                    ...
                    <tbody>
                        ${rows.map(row => `
                            <tr>
                                ...
                                <td>${money(row.rentAmount)}</td>
                                <td>${money(row.utilitiesAdvance)}</td>
                                <td>${money(row.managementMarekAmount)}</td>
                                <td>${money(row.ownerIncomeAmount)}</td>
                                <td>${money(row.taxAmount)}</td>
                                <td>
                                    ${row.paymentStatus === 'paid'
                                        ? `<button type="button" class="rental-row-action" data-rental-undo="${row.tenantId}">Cofnij</button>`
                                        : `<button type="button" class="rental-row-action primary" data-rental-paid="${row.tenantId}">Zaksięguj</button>`}
                                </td>
                            </tr>
                        `).join('')}
                    </tbody>
                    ...
                    return `
                        <section class="rental-year-month">
                            <div class="rental-year-month-head">
                                ...
                                <span>Podatek narastająco: ${money(summary.taxYearToDate)}</span>
                            </div>
                                    ...
                                    <tbody>
                                        ${(month.rows || []).map(row => `
                                            <tr>
                                                ...
                                                <td>${money(row.rentAmount)}</td>
                                                <td>${money(row.utilitiesAdvance)}</td>
                                                <td>${money(row.utilitiesPaidAmount)}</td>
                                                <td>${money(row.managementMarekAmount)}</td>
                                                <td>${money(row.ownerIncomeAmount)}</td>
                                                <td>${money(row.taxAmount)}</td>
                ...
                <div class="rental-table-wrap">
                    <table class="rental-table">
                        <thead>
                            <tr>
                                <th>Najemca</th>
                                <th>Oczekiwane rocznie</th>
                                <th>Zapłacone rocznie</th>
                                <th>Miesiące po terminie</th>
                                <th>Aliasy do importu</th>
                            </tr>
                        </thead>
                        <tbody>
                            ${tenants.map(tenant => `
    ...
    function renderImportView() {
        ...
        target.innerHTML = `
            <div class="rental-import-box">
                <div>
                    <strong>Import wyciągu bankowego</strong>
                    <span>CSV działa już jako parser reguł. XLSX/PDF są przyjmowane jako bezpieczny punkt wejścia do kolejnego etapu ekstrakcji.</span>
                </div>
                <input type="file" id="rentalBankFileInput" accept=".csv,.txt,.xlsx,.pdf">
    ...
    function bindMonthPaymentActions() {
        document.querySelectorAll('[data-rental-paid]').forEach(button => {
            button.addEventListener('click', async () => {
                await bookTenantPayment(Number(button.dataset.rentalPaid), false);
            });
        });
        document.querySelectorAll('[data-rental-undo]').forEach(button => {
            button.addEventListener('click', async () => {
                await bookTenantPayment(Number(button.dataset.rentalUndo), true);
            });
        });
    }

    async function bookTenantPayment(tenantId, undo) {
        if (!Number.isInteger(tenantId) || tenantId <= 0) {
            return;
        }
        const action = undo ? undoTenantPayment : markTenantAsPaid;
        if (typeof action !== 'function') {
            if (typeof showToast === 'function') {
                showToast('Akcja wpłaty nie jest jeszcze dostępna.', 'warning');
            }
            return;
        }
        try {
            await action(tenantId, getSelectedMonth());
            await renderRentalsPanel();
        } catch (error) {
            console.error('Nie udało się zmienić statusu wpłaty:', error);
            if (typeof showToast === 'function') {
                showToast('Nie udało się zaktualizować wpłaty najemcy.', 'error');
            }
        }
    }
    ...
    function renderBankImportPreview(preview) {
        ...
        results.innerHTML = `
            ...
            ${suggestions.length === 0 ? '<div class="rental-empty"><strong>Brak transakcji do pokazania.</strong><span>Dla XLSX/PDF pełna ekstrakcja będzie kolejnym krokiem; CSV pokaże dopasowania od razu.</span></div>' : `
                <div class="rental-table-wrap">
                    <table class="rental-table">
                        <thead>
                            <tr>
